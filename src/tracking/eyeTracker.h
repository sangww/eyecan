...
class eyeEllipse {
	...
	void draw(float x, float y){
		draw(x,y, getWidth(), getHeight(), getWidth(), getHeight());
	}
	
	void draw(float x, float y, float w, float h, float cameraW, float cameraH){
		...
		glPushMatrix();
			...
			glTranslatef(x, y, 0);
			glScalef( (float)w/(float)cameraW, (float)h/(float)cameraH, 1);
