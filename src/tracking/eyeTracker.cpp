...
void eyeTracker::update(ofxCvGrayscaleImage & grayImgFromCam, float threshold, float minSize, float maxSize,  float minSquareness){
	...
	if (bFoundOne && whoFound != -1){
		...
		int MYN = hullcount;
		//float x[MYN], y[MYN]; //to support vs 2010
		float *x, *y;
		x= new float[MYN];
		y= new float[MYN];
		...
		if (bOk == true){
			...
			//ofxPoint2f ptsForRotation[resolution]; //to support vs 2010
			ofxPoint2f *ptsForRotation= new ofxPoint2f[resolution];
